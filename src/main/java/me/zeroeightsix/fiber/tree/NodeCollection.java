...
public class NodeCollection extends AbstractCollection<ConfigNode> implements Collection<ConfigNode> {
    private final Map<String, ConfigNodeImpl> items = new TreeMap<>();
    @Nullable private final ConfigGroup owner;

    public NodeCollection(@Nullable ConfigGroup owner) {
        this.owner = owner;
    }

    public NodeCollection(@Nullable ConfigGroup owner, Collection<ConfigNode> items) {
        this(owner);
        this.addAll(items);
    }
    ...
    public ConfigNodeImpl get(String name) {
